...
let rec run_game () =
	...
	if (init_command = "HELP") then (SaveGame.print_help ();run_game ())
	else if init_command = "LEADERBOARD" then (SaveGame.display_high_score ();run_game ())
	else if (String.sub init_command 0 (String.index init_command ' ') = "STATS")
		then
		let last_space = (String.rindex init_command ' ') + 1 in
	...
	else if init_command = "READY" then (
	...
	while (not (is_winner !player_list)) do
		...
		it needs access to those cards*)
			...
			let (updated_players_after_turn: Player.player list) = update_score_after_turn post_exchange_players turn_result [] in
